...
//   export default schema;
  ...
  function main(){
    ...
    for (let entry of schema.entries()) {
      let key = toNum(toArr(entry[0].toString(2).substring(1)));
      rebuild[key] = entry[1];
    }

    if(true){
        fs.writeFile('./schema.json', JSON.stringify(rebuild,null,2), err => {
            if (err) {
              console.error(err)
              return
            }
